  ...
  return (
    <div className="h-full overflow-auto bg-[var(--bg)] font-ui text-[12px]">
      <div className="sticky top-0 z-10 flex h-[44px] items-center gap-2 border-b border-[var(--border)] bg-[var(--panel)] px-3">
        ...
        <button onClick={() => setCreating(true)} className="ml-auto flex h-[30px] items-center gap-1.5 rounded-md bg-[var(--accent)] px-3 font-medium text-white"><Plus size={14} />{activeSelection ? 'Create from selection' : 'Add primer'}</button>
      </div>
      ...
      <section className="m-3 border border-[var(--border)] bg-[var(--panel)]">
        ...
        <div className="grid grid-cols-[1fr_1fr_auto] gap-2 p-3">
          ...
          <button onClick={runPCR} className="h-[34px] rounded-md bg-[var(--accent)] px-3 font-medium text-white">Simulate PCR</button>
        </div>
