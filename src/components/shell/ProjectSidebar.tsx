...
import { Circle, CircleDot, Dna, FileUp, Search, X, Trash2 } from 'lucide-react';
...
export function ProjectSidebar() {
  ...
  return (
    <div className="flex h-full flex-col bg-[var(--panel-muted)] text-[var(--text)] font-ui">
      <div className="shrink-0 p-3 pb-2 text-[11px] font-semibold text-[var(--text-secondary)]">Project</div>
      ...
      {query ? (
        <div className="min-h-0 flex-1 overflow-y-auto px-2">
          <div className="px-1 pb-1 text-[11px] font-medium text-[var(--text-muted)]">Results</div>
          {results.map((result, index) => (
            <button key={`${result.kind}-${result.documentId}-${index}`} onClick={() => openResult(result)} className="w-full rounded px-2 py-1.5 text-left hover:bg-[var(--panel)]">
              ...
              <div className="truncate text-[11px] capitalize text-[var(--text-muted)]">{result.kind} · {result.detail}</div>
            </button>
          ))}
        ...
        <>
          <div className="shrink-0 px-3 pb-1 text-[11px] font-medium text-[var(--text-muted)]">DNA Sequences</div>
              ...
              return (
                <div key={document.id} className={`group flex w-full items-center gap-2 rounded border-l-2 px-2 py-1.5 text-left text-[12px] transition-colors ${active ? 'border-[var(--accent)] bg-[var(--accent-soft)] text-[var(--text)]' : 'border-transparent text-[var(--text-muted)] hover:bg-[var(--panel)] hover:text-[var(--text)]'}`}>
                  <button className="flex min-w-0 flex-1 items-center gap-2 text-left" onClick={() => setActiveDocument(document.id)}>
                    <span className={`shrink-0 ${active ? 'text-[var(--accent)]' : 'text-[var(--text-muted)]'}`}>{circular ? (active ? <CircleDot size={11} /> : <Circle size={10} />) : <Dna size={12} />}</span>
                    <span className="flex min-w-0 flex-col overflow-hidden">
                      <span className={`truncate ${active ? 'font-medium' : ''}`}>{document.name}</span>
                      <span className="truncate text-[11px] opacity-70">{formatLength.format(document.length)} bp · {circular ? 'Circular' : 'Linear'}</span>
                    </span>
                  </button>
                  <button 
