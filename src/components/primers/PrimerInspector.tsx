...
export function PrimerInspector({ document, primer }: { document: SequenceDocument; primer: Primer }) {
  ...
  return (
    <div className="flex flex-col gap-3 text-[12px]">
      <div><h2 className="text-[13px] font-semibold">{primer.name}</h2><div className="text-[11px] uppercase tracking-wider text-[var(--text-muted)]">Primer</div></div>
      ...
      <div className="border-t border-[var(--border)] pt-3"><button onClick={() => setEditing(true)} className="text-[var(--accent)] hover:underline">Edit primer</button></div>
