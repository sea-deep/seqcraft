  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[var(--bg)] font-ui text-[12px]">
      <div className="flex min-h-[48px] shrink-0 items-center gap-2 overflow-x-auto border-b border-[var(--border)] bg-[var(--panel)] px-3 scrollbar-none">
        ...
        {ready && <div className="ml-auto flex shrink-0 rounded-md border border-[var(--border)] p-0.5"><button aria-pressed={view === 'report'} onClick={() => setView('report')} className={`h-7 rounded px-2.5 ${view === 'report' ? 'bg-[var(--accent-soft)] font-medium text-[var(--accent)]' : 'text-[var(--text-muted)] hover:text-[var(--text)]'}`}>Summary</button>{canShowCircular && <button aria-pressed={view === 'circular'} onClick={() => setView('circular')} className={`h-7 rounded px-2.5 ${view === 'circular' ? 'bg-[var(--accent-soft)] font-medium text-[var(--accent)]' : 'text-[var(--text-muted)] hover:text-[var(--text)]'}`}>Map</button>}<button aria-pressed={view === 'alignment'} onClick={() => setView('alignment')} className={`h-7 rounded px-2.5 ${view === 'alignment' ? 'bg-[var(--accent-soft)] font-medium text-[var(--accent)]' : 'text-[var(--text-muted)] hover:text-[var(--text)]'}`}>Alignment</button></div>}
