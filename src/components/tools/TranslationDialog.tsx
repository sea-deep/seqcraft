...
export function TranslationDialog({ document, selection, open, onOpenChange }: { document: SequenceDocument; selection: { start0: number; end0Exclusive: number }; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[620px] p-5 gap-4">
        <DialogHeader><DialogTitle>Translate selection</DialogTitle></DialogHeader>
        <div className="flex gap-2 text-[12px]"><select aria-label="Translation strand" value={strand} onChange={event => setStrand(Number(event.target.value) as 1 | -1)} className="h-[34px] rounded-md border border-[var(--border)] bg-[var(--bg)] px-2"><option value={1}>Forward strand</option><option value={-1}>Reverse strand</option></select><select aria-label="Translation frame" value={frame} onChange={event => setFrame(Number(event.target.value))} className="h-[34px] rounded-md border border-[var(--border)] bg-[var(--bg)] px-2"><option value={1}>Frame 1</option><option value={2}>Frame 2</option><option value={3}>Frame 3</option></select><span className="self-center text-[var(--text-muted)]">{selected.length} nt → {aminoAcids.length} aa</span></div>
